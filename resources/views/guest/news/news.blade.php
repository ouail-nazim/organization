		...
		@foreach ($news as $item)
			<div class="flex flex-col my-3">
				...
				<div class="{{textSide()}} @if(isRTL())flex flex-row-reverse @else flex-row @endif" >
					<a href="{{route('news.read',['news'=>$item->id])}}"
			...
			<nav class="flex flex-row justify-center">
				<ul class="pagination">
					...
					<li class="page-item   @if($news->currentPage() == 1) disabled @endif ">
                        <a class="page-link @if($news->currentPage() != 1) text-red hover:bg-gray-200 hover:text-red-400 @endif"
                             href="{{ $news->previousPageUrl() }}">{{__('previous')}}</a>
                    </li>

					<li class="page-item disabled" aria-current="page">
						<p class="page-link text-red ">
							{{__('page')}} {{ $news->currentPage() }} {{__('of')}}  {{ $news->lastPage() }}
						</p>
					</li>
					<li class="page-item @if($news->currentPage() == $news->lastPage()) disabled @endif">
                        <a  class="page-link @if($news->currentPage() != $news->lastPage()) text-red hover:bg-gray-200 hover:text-red-400" @endif" 
