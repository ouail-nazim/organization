...
@section('main')
    <main class="container p-2">
        <div class="text-gray-500 py-1 {{textSide()}} text-xl capitalize subpixel-antialiased">
            {{getDateString($meeting->created_at)}}
        </div>
        ...
        <section class="my-3">
            <div class="flex flex-col my-2">
                <div class="w-full h-72 flex @if(isRTL()) flex-row-reverse @else  flex-row @endif  ">
                    <img src="{{$meeting->cover}}" class="w-4/5 h-full shadow-md object-center rounded" alt="news-image">
                </div>
                <p class="py-4 text-black font-base text-lg {{textSide()}}">
                    @if(isRTL())
                        {{$meeting->ar_description}}
                    @else 
                        {{$meeting->en_description}}
                    @endif
                </p>        
                <div class="{{textSide()}} @if(isRTL())flex flex-row-reverse @else flex-row @endif" >
                    <a href="{{route('meetings')}}"
                        ...
                        <span class="group-hover:text-gray-100 ">{{__("meetings")}}</span>
                    </a>
                    <button type="button"
                            ...
                            data-toggle="modal" data-target="#meeting{{$meeting->id}}">
                            <i class="fas fa-share    "></i>
                    </button>
                    <!-- Modal -->
                    <div class="modal fade" id="meeting{{$meeting->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                ...
                                <div class="modal-body">
                                    <div class="form-group">
                                        ...
                                        <input readonly 
                                            ...
                                            id="recipient-name" 
                                            value="{{asset("/meetings/".$meeting->id)}}">
                                    </div>
                                </div>
